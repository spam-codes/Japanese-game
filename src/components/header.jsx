...
function Header({changeScriptFunction, activeMode, theme, isMenuOn, changeIsMenuOn, script}){
    ...
    return <div className={`${headerBackground} ${headerText} absolute top-0 w-screen h-15 flex justify-end items-center`}>
        ...
        <div className="flex md:hidden">
            ...
            >
                <button className={`${buttonStyle} ${activeMode==='hiragana'?activeStyle:''} text-start`} onClick={()=>changeScriptFunction('hiragana')} >Hiragana</button>
                <button className={`${buttonStyle} ${activeMode==='katakana'?activeStyle:''} text-start`} onClick={()=>changeScriptFunction('katakana')}>Katakana</button>
                <button className={`${buttonStyle} ${activeMode==='hiraganaCombination'?activeStyle:''} text-start`} onClick={()=>changeScriptFunction('hiraganaCombination')}>HR-Combo</button>
                <button className={`${buttonStyle} ${activeMode==='katakanaCombination'?activeStyle:''} text-start`} onClick={()=>changeScriptFunction('katakanaCombination')}>KT-Combo</button>
            </motion.div >}
            </AnimatePresence>
        </div>
    </div>
